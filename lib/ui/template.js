...
Template.prototype = {
    ...
    _binds: function (nodes) {
        ...
        this.each(nodes, function (item) {
            if (item.getAttribute && item.getAttribute('tw-for')) return;
    ...
    update: function () {
        ...
        this.each(this.root.querySelectorAll('[tw-for]'), function (node) {
    ...
    get: function (key, scope) {
        ...
        var arr = key.split('.'), i=0, l=arr.length, obj=scope || this.scope;
        for(i; i<l; i++) {
            if (!obj) {
                obj='';
                break;
