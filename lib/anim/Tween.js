...
var
    ...
    Tween = function (tar, to, from, time, ease) {
        if(!to && !from){ throw 'invalid parameter!'; }
...
Tween.prototype = {
    ...
    {
        if (this.status === 0) this.__init(this.tar, this.to, this.from);
    ...
    __init: function (tar, to, from)
    {
        ...
        if (tar.hasInit) tar.init();
        for(p in obj) {
            func = this['__' + typeof tar[p]];
            if (func) {
    ...
    __function: function (tar, p, to, from, params)
    {
        switch(typeof params) {
            case 'boolean':
                this.__funcWithBoolean(tar, p, to);
                return;
            case 'string':
                this.__funcWithString(tar, p, to);
                return;
            case 'object':
    ...
    __funcWithString: function (tar, p, to) {
        var d = to[p];
        this['_t' + this.len] = noop;
        this['_f' + this.len] = this.__fixfunc;
        this['_' + this.len] = {t: tar, p: tar[p], d: d};
    },
    __funcWithBoolean: function (tar, p, to) {
        var d = to[p];
        this['_t' + this.len] = noop;
        this['_f' + this.len] = this.__fixfunc;
        this['_' + this.len] = {t: tar, p: tar[p], d: d};
    ...
    __functionWithObject: function (tar, p, to, from, params)
    {
        ...
        for (var n in params) {
            ...
            else if (typeof params[n] === 'string') {
                /**
                 * cssの%によるアニメーションの対応を追加したい。
                 * スタイルを取得するgetPropertyValue()が必ずpxで値が返されるので、
                 * 自前で%の値を計算する必要がある
                 * console.log(n, params[n], d[n]);
                 */
                if (d[n] === undefined) d[n] = func.call(tar, n);
            }
            else {
        ...
        this['_f' + this.len] = this.__fixfunc;
        this['_' + this.len] = {t: tar, p:tar[p], c: c, b: b, d: d, s:{}, g:g};
    },
    __boolean: function (tar, p, to) {
        var d = to[p];
        this['_t' + this.len] = noop;
    ...
    __getColor: function (b, c) {
        var t= this.t, du=this.du, ease=this.ease;
        return 'rgba('
            + parseInt(ease(t, b.r, c.r, du)) + ','
            + parseInt(ease(t, b.g, c.g, du)) + ','
            + parseInt(ease(t, b.b, c.b, du)) + ','
            + ease(t, b.a, c.a, du) + ')';
    },
